...
public class OutfitStandDrawProvider_Decorations : OutfitStandDrawProvider
{
    ...
    public override void CollectDraws(OutfitStandDrawContext context)
    {
        ...
        foreach (var pair in decorativeComp.Decorations)
        {
            ...
            var graphic = BuildGraphic(decoration, settings, useBodyType, graphicBodyType);
            if (graphic == null)
            {
                continue;
            }
    ...
    /// <summary>
    /// The same resolution PawnRenderNode_AttachmentExtraDecoration.GraphicFor does, with no wearer
    /// so no gendered art.
    /// </summary>
    private static Graphic BuildGraphic(ExtraDecorationDef decoration, DecorationSettings settings, bool useBodyType, BodyTypeDef bodyType)
    {
